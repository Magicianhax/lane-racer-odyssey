...
const CarSelectionDialog: React.FC<CarSelectionDialogProps> = ({
  ...
  const carOptions = [
    { name: "Sports Car", url: "/playercar.png" },
    { name: "Super Car", url: "/playercar2.png" }
  ];

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md max-h-[90vh] bg-gradient-to-b from-[#0b131e] via-[#172637] to-[#1f3a57] text-white border-[#91d3d1]/20 p-6 overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-4 mt-2">
          {carOptions.map((car) => (
            <div 
              key={car.url}
              className={`flex flex-col items-center p-4 rounded-xl cursor-pointer transition-all hover:scale-105 ${
                selectedCar === car.url 
